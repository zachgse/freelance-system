...
async function fetchFreelances(){
    try{
        const response = await api.get('/users/freelances',{
...
//   console.log(`${action} clicked for ID: ${id}`);\
    if (action === "View") router.push(`/projects/${slug}`);
...
<template>
    ...
    <div v-else>
        <div class="max-w-[1300px] border-1 border-gray-300 w-full h-auto min-h-96 flex flex-col justify-center mx-auto my-12 p-4">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg h-full min-h-full">
                <table class="w-full min-h-96 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(freelance,index) in freelances" :key="freelance?.freelance_project_details?.id"
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{new Date(freelance?.freelance_project_details?.date_posted).toLocaleDateString("en-US", { year:'numeric',month:'long',day:'numeric' })}}
                            </th>
                            <td class="px-6 py-4">
                                {{ freelance?.freelance_project_details?.title }}
                            </td>
                            <td class="px-6 py-4">
                                {{ freelance?.freelance_project_details?.number_of_total_proposals }}
                            </td>
                            <td class="px-6 py-4">
                                {{ freelance?.freelance_project_details?.status }}
                            </td>
                            <td class="px-6 py-4 relative">
                                <!-- Dropdown Button -->
                                <button 
                                @click="toggleDropdown(freelance?.freelance_project_details?.id)" 
                                ...
                                <!-- Dropdown Menu -->
                                <div 
                                v-show="openDropdownId === freelance?.freelance_project_details?.id"
                                class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50"
                                >
                                <ul class="p-2 text-sm text-gray-700">
                                    <li @click="handleAction('View', freelance?.freelance_project_details?.slug)" class="p-2 hover:bg-gray-100 cursor-pointer">
                                    View
                                    </li>
                                    <li @click="handleAction('Edit', freelance?.freelance_project_details?.id)" class="p-2 hover:bg-gray-100 cursor-pointer">
                                    Edit
                                    </li>
                                    <li @click="handleAction('Delete', freelance?.freelance_project_details?.id)" class="p-2 hover:bg-gray-100 cursor-pointer text-red-600">
                                    Delete
                                    </li>
                                </ul>
                                </div>
